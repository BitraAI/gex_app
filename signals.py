...
def generate_recommendations(
    ...
    if strategy in ("Long Calls",):
        ...
        candidates = [
            ...
            and (e.get("open_interest", 0) or 0) > 500
        ]
        if not candidates:
            recs.append(f"No OTM calls in DTE {dte_min}-{dte_max} range with OI > 500.")
        else:
    ...
    if strategy in ("Long Puts",):
        ...
        if not candidates:
            recs.append(f"No OTM puts in DTE {dte_min}-{dte_max} range with OI > 500.")
        else:
            cand_exps = {e["expiration"] for e in candidates}
            best_exp = min(cand_exps, key=lambda e: exp_vrp.get(e, 0))
            if iv_skew is None or iv_skew >= 0:
                recs.append(f"IV Skew {f'{iv_skew:+.2%}' if iv_skew is not None else 'N/A'} — not bearish for puts; skip Long Puts.")
            elif exp_vrp.get(best_exp, 0) >= -5:
    ...
    if strategy in ("Short Calls",):
        ...
        candidates = [
            ...
            if e.get("type") == "CALL" and (e.get("strike", 0) or 0) > spot
            and dte_min <= (e.get("days_to_exp", 0) or 0) <= dte_max
            and (e.get("open_interest", 0) or 0) > 1000
        ]
        if not candidates:
            recs.append(f"No OTM calls in DTE {dte_min}-{dte_max} range with OI > 1000.")
        else:
            cand_exps = {e["expiration"] for e in candidates}
            best_exp = max(cand_exps, key=lambda e: exp_vrp.get(e, 0))
            if iv_skew is None or iv_skew >= 0:
                recs.append(f"IV Skew {f'{iv_skew:+.2%}' if iv_skew is not None else 'N/A'} — not bearish for calls; skip Short Calls.")
            elif exp_vrp.get(best_exp, 0) <= 5:
    ...
    if strategy in ("Short Puts",):
        ...
        if not candidates:
            recs.append(f"No OTM puts in DTE {dte_min}-{dte_max} range with OI > 1000.")
        else:
            cand_exps = {e["expiration"] for e in candidates}
            best_exp = max(cand_exps, key=lambda e: exp_vrp.get(e, 0))
            if iv_skew is None or iv_skew <= 0:
                recs.append(f"IV Skew {f'{iv_skew:+.2%}' if iv_skew is not None else 'N/A'} — not bullish for puts; skip Short Puts.")
            elif exp_vrp.get(best_exp, 0) <= 5:
...
def build_strategy_alerts(
    ...
    enable_long_calls: bool = True,
    enable_long_puts: bool = True,
    enable_short_calls: bool = True,
    enable_short_puts: bool = True,
) -> list[str]:
    ...
    # Build alerts based on enabled strategies
    if enable_long_calls:
        ...
        if buy_recs:
            alerts.append("Long Calls:")
            for r in buy_recs[:3]:
                alerts.append(f"  {r}")

    if enable_long_puts:
        buy_recs = [r for r in generate_recommendations(sd2, spot, strategy="Long Puts", all_data=sd2, rv=rv, call_wall=analytics.get("call_wall"), put_wall=analytics.get("put_wall"), iv_skew=analytics.get("iv_skew"), ssvi_surface=ssvi_surf, ssvi_tte=ir_tte, bias=bias) if "No strong" not in r and "No OTM" not in r and "skip" not in r and "GEX Bias" not in r]
        if buy_recs:
            alerts.append("Long Puts:")
            for r in buy_recs[:3]:
                alerts.append(f"  {r}")

    if enable_short_calls:
        ...
        if sell_recs:
            alerts.append("Short Calls:")
            for r in sell_recs[:3]:
                alerts.append(f"  {r}")

    if enable_short_puts:
        sell_recs = [r for r in generate_recommendations(sd2, spot, strategy="Short Puts", all_data=sd2, rv=rv, call_wall=analytics.get("call_wall"), put_wall=analytics.get("put_wall"), iv_skew=analytics.get("iv_skew"), ssvi_surface=ssvi_surf, ssvi_tte=ir_tte, bias=bias) if "No strong" not in r and "No OTM" not in r and "skip" not in r and "GEX Bias" not in r]
        if sell_recs:
            alerts.append("Short Puts:")
            for r in sell_recs[:3]:
                alerts.append(f"  {r}")
